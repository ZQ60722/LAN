document.addEventListener('DOMContentLoaded', function() {
    ...
    searchInput.addEventListener('input', function (e) {
        ...
        if (searchTerm) {
            fetch(`http://127.0.0.1:8000/search?term=${encodeURIComponent(searchTerm)}`)
            ...
            .then(data => {
                if (data.success) {
                    ...
                    if (data.fileName) {
                        ...
                        // 上传成功但没有返回文件路径
                        showToast('上传成功但未获取到文件路径');
                    }
                } else {
                    // 上传失败，显示后端返回的具体错误信息
                    showToast(data.message || '文件上传失败');
                }
            })
            .catch(error => {
    ...
    function performRecognition(fileName) {
    ...
    function renderRecognitionResults(results) {
        ...
        results.forEach(item => {
            // item 是一个字典，如 {'材料': 'Material'}
            // 我们需要获取它的键和值
            const chineseTerm = Object.keys(item)[0];
            const englishTranslation = item[chineseTerm];

            html += `
        <div class="translation-item">
            <p class="original">原文：${chineseTerm}</p>
            <p class="translated">识别/翻译：${englishTranslation}</p>
        </div>
    `;
        });
